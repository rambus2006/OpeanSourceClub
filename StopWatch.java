...
private Panel createCenterPanel(){
		...
		Panel centerPanel = new Panel(gridLayout);		//화면 패널 설정
		...
		JLabel title = new JLabel("스탑워치");		//패널 제목
		title.setForeground(Color.white);
		title.setFont(new Font("Gothic", Font.BOLD, 30));	//글씨체 설정
		constraints.gridx = 0;
		constraints.gridy = 0;
...
private Panel createSouthPanel(){
		...
		stop= new JLabel("INITIALIZATION");		//초기화 레이블, stop버튼이 초기화되는 기능이 되어 있어서 이름을 바꿔주었습니다. 
		
		start.setOpaque(true);	//스톱워치의 움직이는 숫자를 보여줌
		stop.setOpaque(true);
